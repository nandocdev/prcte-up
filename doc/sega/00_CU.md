...
#### **CU1: Registrar Trabajo de Extensión** **COMPLETADO**
...
*   **Actor Principal:** Profesor
*   **Precondiciones:** El Profesor tiene un Trabajo de Extensión en estado "Trabajo Rechazado (para Subsanación)".
*   **Flujo Principal:**
    1.  El Profesor accede a la lista de sus Trabajos de Extensión.
    2.  Selecciona un trabajo en estado "Trabajo Rechazado (para Subsanación)".
    3.  El sistema muestra el trabajo con los comentarios de rechazo de la última instancia revisora.
    4.  El Profesor edita el trabajo para aplicar las subsanaciones requeridas.
    5.  El Profesor guarda los cambios.
    6.  El Profesor selecciona la opción "Reenviar a Revisión".
    7.  El sistema cambia el estado del trabajo a "Pendiente Revisión (Coordinación)".
    8.  El sistema registra la transición en el `work_status_history`.
    9.  El sistema envía una notificación al Coordinador de Extensión.
*   **Postcondiciones:** El Trabajo de Extensión subsanado vuelve al inicio del flujo de aprobación.
...
*   **Precondiciones:** El Coordinador de Extensión ha iniciado sesión y tiene Trabajos de Extensión en estado "Pendiente Revisión (Coordinación)".
*   **Flujo Principal:**
    ...
    3.  El sistema muestra el formulario completo del trabajo, evidencias, historial y comentarios.
    4.  El Coordinador revisa el trabajo.
    5.  Si el trabajo cumple con los requisitos iniciales, el Coordinador selecciona "Aprobar y Elevar a Decano/Director".
    6.  El sistema cambia el estado del trabajo a "Pendiente Revisión (Decano/Director)".
    7.  El sistema registra la transición en el `work_status_history` y envía una notificación al Decano/Director y al Profesor.
*   **Flujos Alternos:**
    *   **CU10: Solicitar Correcciones al Profesor:** El Coordinador puede solicitar ajustes menores.
    *   **CU11: Rechazar Trabajo para Subsanación:** El Coordinador puede rechazar el trabajo si requiere cambios significativos.
...
*   **Precondiciones:** El Decano/Director ha iniciado sesión y tiene Trabajos de Extensión en estado "Pendiente Revisión (Decano/Director)".
*   **Flujo Principal:**
    1.  El Decano/Director accede a su bandeja de trabajos pendientes.
    2.  Selecciona un Trabajo de Extensión para revisión.
    3.  El sistema muestra el formulario completo, evidencias, historial de revisiones previas y comentarios.
    4.  El Decano/Director revisa el trabajo.
    5.  Si el trabajo cumple con los requisitos, el Decano/Director selecciona "Aprobar y Elevar a VIEX".
    6.  El sistema cambia el estado del trabajo a "Pendiente Evaluación (VIEX)".
    7.  El sistema registra la transición en el `work_status_history` y envía una notificación al Personal VIEX y al Profesor.
*   **Flujos Alternos:**
    *   **CU10: Solicitar Correcciones al Profesor:** El Decano/Director puede solicitar ajustes menores.
    *   **CU11: Rechazar Trabajo para Subsanación:** El Decano/Director puede rechazar el trabajo si requiere cambios significativos.
*   **Postcondiciones:** El Trabajo de Extensión avanza al siguiente nivel de revisión o es devuelto para correcciones/subsanación.

#### **CU9: Evaluar y Aprobar Trabajo - VIEX**
*   **Actor Principal:** Personal VIEX (Evaluador VIEX)
*   **Precondiciones:** El Personal VIEX ha iniciado sesión y tiene Trabajos de Extensión en estado "Pendiente Evaluación (VIEX)".
*   **Flujo Principal:**
    1.  El Personal VIEX accede a su bandeja de trabajos pendientes.
    2.  Selecciona un Trabajo de Extensión para evaluación.
    3.  El sistema muestra todos los detalles del trabajo y el historial de revisiones.
    4.  El Personal VIEX asigna evaluadores al trabajo (mediante `work_evaluators`).
    5.  Los evaluadores completan la evaluación utilizando los `evaluation_criteria`, registrando `work_evaluations` y `evaluation_details`.
    6.  El Personal VIEX (o la Comisión VIEX) revisa las evaluaciones y toma una decisión final.
    7.  Si la evaluación es positiva, el Personal VIEX selecciona "Aprobar Evaluación".
    8.  El sistema cambia el estado del trabajo a "Aprobado Internamente (por VIEX)".
    9.  El sistema registra la transición en el `work_status_history` y envía una notificación al Profesor.
*   **Flujos Alternos:**
    *   **CU10: Solicitar Correcciones al Profesor:** El Personal VIEX puede solicitar ajustes menores tras la evaluación.
    *   **CU11: Rechazar Trabajo para Subsanación:** El Personal VIEX puede rechazar el trabajo si requiere cambios significativos tras la evaluación.
...
*   **Precondiciones:** El Actor está revisando un Trabajo de Extensión.
*   **Flujo Principal:**
    1.  El Actor identifica ajustes menores o aclaraciones necesarias en el trabajo.
    2.  El Actor selecciona la opción "Solicitar Correcciones".
    3.  El sistema solicita al Actor que ingrese comentarios detallados sobre las correcciones requeridas.
    4.  El sistema cambia el estado del trabajo a "Correcciones Solicitadas (al Profesor)".
    5.  El sistema registra la transición en el `work_status_history` y los comentarios en `work_comments` (con visibilidad para el profesor).
    6.  El sistema envía una notificación al Profesor.
*   **Postcondiciones:** El Trabajo de Extensión espera ajustes por parte del Profesor.
...
*   **Flujo Principal:**
    ...
    4.  El sistema cambia el estado del trabajo a "Trabajo Rechazado (para Subsanación)".
    5.  El sistema registra la transición en el `work_status_history` y los comentarios en `work_comments` (con visibilidad para el profesor).
    6.  El sistema envía una notificación al Profesor y al Coordinador de Extensión.
*   **Postcondiciones:** El Trabajo de Extensión regresa al Profesor para subsanación, iniciando un nuevo ciclo de revisión al ser reenviado.

### **Módulo: Gestión de Certificaciones**

#### **CU12: Emitir Certificación de Trabajo**
*   **Actor Principal:** Personal VIEX (Administrativo VIEX)
*   **Precondiciones:** El Personal VIEX tiene Trabajos de Extensión en estado "Aprobado Internamente (por VIEX)".
*   **Flujo Principal:**
    1.  El Personal VIEX accede a la bandeja de trabajos "Listos para Certificar".
    2.  Selecciona uno o varios trabajos.
...
*   **Precondiciones:** El Trabajo de Extensión está en estado "Trabajo Certificado" y el Actor tiene los permisos para descargarla.
*   **Flujo Principal:**
    ...
    4.  El sistema proporciona el archivo PDF.
